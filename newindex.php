...
<script>
  ...
  function selectStation(stationId) {
    ...
    if (selectEl.value !== stationId.toString()) {
  ...
  // Hook dropdown to shared selectStation function
  document.getElementById('stationSelect').addEventListener('change', function () {
    selectStation(this.value);
  });

  // Auto-select current station on page load (from URL if available)
  window.addEventListener('DOMContentLoaded', () => {
    const urlParams = new URLSearchParams(window.location.search);
    const initialStationId = urlParams.get('station_id') || stations[0].id;
